...
class ImageDataset_full(Dataset):
    def __init__(self, label_path, cfg, mode='train'):
        ...
        """
        self.cfg = cfg
        self.disease = np.array(self.cfg.disease_classes)
        self._label_header = None
        df = pd.read_csv(label_path)
        self.class_dist = []
        for class_name in self.cfg.disease_classes:
            self.class_dist.append(sum(df[class_name].to_list()))
        self._image_paths = []
        self._labels = []
        self._mode = mode
        ...
        with open(label_path) as f:
            ...
            for line in f:
                ...
                image_path = os.path.join(
                    '/media/tungthanhlee/DATA/tienthanh/assigned dicom', fields[0]+'.dcm')
    ...
    def __getitem__(self, idx):
        # image = cv2.imread(self._image_paths[idx], 0)
        image = read_xray(self._image_paths[idx])
        image = Image.fromarray(image)
        if self._mode == 'train':
            image = GetTransforms(image, type=self.cfg.use_transforms_type)
        image = np.array(image)
        ...
        if not (self.cfg.type == 'pediatric'):
            labels = labels[self.disease == 'No finding']
